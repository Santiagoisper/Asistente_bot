...
function buildDocx(spec: StudySpec, specVersion: number, specStatus: string): Document {
  ...
  const endpointCols = [1200, 3913, 3913] // type, objective, endpoint
  ...
  // ── Visits table ──
  const visitCols = [1500, 1500, 900, 900, contentWidth - 4800]
    ...
    ...spec.visits.map((v) =>
      new TableRow({
        children: [
          ...
          bodyCell(v.day != null ? String(v.day)          : '—',         visitCols[2], true),
          bodyCell(v.windowDays != null ? `±${v.windowDays}d` : '—',    visitCols[3], true),
  ...
  const visitRows = spec.visits
    .map((v) => `
      <tr>
        ...
        <td class="mono">${v.day != null ? v.day : '—'}</td>
        <td class="mono">${v.windowDays != null ? `±${v.windowDays}d` : '—'}</td>
